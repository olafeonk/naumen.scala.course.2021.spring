...
object Exercises {
  ...
  def sumByFunc(
                 leftVec0: Vector2D,
                 leftVec1: Vector2D,
                 foo: (Vector2D, Vector2D) => Double,
                 rightVec0: Vector2D,
                 rightVec1: Vector2D
               ): Double = foo(leftVec0, leftVec1) + foo(rightVec0, rightVec1)
  ...
  */
  ...
  val balls: Map[String, (Int, Double)] = {
    ...
    def getMass(ball: (String, (Int, Double))): (String, Double) =
      (ball._1, Math.pow(ball._2._1, 3) * ball._2._2)

    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
      ballsArray.map(elem => getMass(elem)).toSeq.sortBy(ball => ball._2).map(ball => ball._1)
}
